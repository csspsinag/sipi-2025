...
export function chart(position: Positions) {
	...
	const initRootHead = `${root.data.name}`;
				...
				mdlLabel
					...
					.attr(
						...
						d.depth === 1
							...
							: d.depth === 2
								? d3.interpolateLab(
										departmentScale(d.data.name),
										partyScale(d.parent.data.party)
									)(0.5)
								: d3.interpolateLab(
										departmentScale(d.parent.data.name),
										partyScale(d.parent.parent.data.party)
					...
					.text(
						(d.height < 2
							? (d.data.display
									? d.data.display
									: departmentNameScale(d.data.name) || d.data.name) + ': '
							: '') +
							percentage +
							'%'
					);
	...
	function clicked(event, p) {
		...
		if (chartContext === root) {
			...
			rootHead = `${p.parent.data.name} votes ${p.depth === 1 ? 'for' : 'in'} ${departmentNameScale(p.data.name) || p.data.name}`;
